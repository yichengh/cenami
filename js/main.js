...
;(function(window) {
	...
	var support = {transitions : Modernizr.csstransitions},
		...
		buyCtrl = document.querySelector('.action--buy'),
	...
	function initEvents() {
		...
		var onBuyCtrlClick = function() {
			console.log("buy");
			//console.log(classie.get('row__seat--selected'));
			var array = new Array();
			planseats.forEach(function(planseat) {
				if( classie.has(planseat, 'row__seat--selected') ) {
					console.log(planseat.attributes[2].value);
					array.push(planseat.attributes[2].value);
				}
			});
			console.log(array);
			//$.post("test.php", { seats: array } );
			// $.ajax(function(){
			// 	type:"POST",
			// 	dataType:"json",
			// 	url:"test.php",//请求页面
		...
		buyCtrl.addEventListener('click', onBuyCtrlClick);
  ...
  function work() {
    ...
    var id = $("#get_id").html();
    console.log(id);
    $.post("get_seat_sold.php", { showing_id: id},
      function(data){
        ...
        var S = new Set();
        for (var i in A) {
          var x = parseInt(A[i]["x"]);
          var c = String.fromCharCode(x + 64) + A[i]["y"];
          S.add(c);
        }
        var html = [];
        for (var x = 1; x <= 18; x++) {
          ...
          for (var y = 1; y <= 18; y++) {
            var c = String.fromCharCode(x + 64) + y;
            if (S.has(c)) {
              html.push("<div class='row__seat row__seat--reserved'></div>");
            } else {
              html.push("<div class='row__seat tooltip' data-tooltip='" + c + "'></div>");
            }
          }
          html.push("</div>");
        }
        $("#seats").html(html.join(''));
    ...
    $.post("get_seat_sold.php", { showing_id: id},
      function(data){
        ...
        for (var i in A) {
        	...
        	var c = String.fromCharCode(x + 64) + A[i]["y"];
        	console.log(c);
        	//$("#" + c).html("<div class=\"row__seat row__seat--reserved\"></div>");
        	$("#" + c).attr("class", "row__seat row__seat--reserved");
   			$("#" + c).removeAttr("tooltip");
        }
        console.log("finished");
